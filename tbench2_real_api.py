#!/usr/bin/env python3
"""Terminal-Bench 2.0 Full Benchmark - REAL API"""
...
print("🧪 Terminal-Bench 2.0 - REAL API MODE")
print("="*60)
...
for i, task in enumerate(tasks, 1):
    ...
    try:
        result = subprocess.run(
            ["./bin/eai", "agent", "--max-loops", "3", short_instr],
            capture_output=True,
            text=True,
            timeout=120,
            env=env
...
print(f"\n{'='*60}")
print(f"📊 BENCHMARK RESULTS (REAL API)")
print(f"{'='*60}")
print(f"Total:    {total}")
...
output = {
    "benchmark": "Terminal-Bench 2.0 (REAL API)",
    "timestamp": datetime.now().isoformat(),
    "api": "minimax",
    "summary": {"total": total, "success": success, "failed": failed, "timeout": timeout, "rate": rate},
    "tasks": results
}
with open("tbench2_real_api_results.json", "w") as f:
    json.dump(output, f, indent=2)
print(f"\n💾 Results saved to tbench2_real_api_results.json")
